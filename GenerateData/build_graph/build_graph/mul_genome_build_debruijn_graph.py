...
class build_debruijn_graph:
        ...
        with open(self.gfa_path, 'r') as file:
            for line in file:
                ...
                if line.startswith("L"):
                    ...
                    edge_for_label[(pre_node_index, suf_node_index)] = edge

                    edges[(pre_node_index, suf_node_index)] = [edge_len, edge_km, edge_occurrence, pre_node_singal, suf_node_singal, over_lap_len]
    ...
    def process_edge(self, args):
        """单个任务函数"""
        node_pire, edge, reference = args
        for seq in reference:
            if edge in seq:
                return (node_pire, 1.0)
        return (node_pire, 0.0)
    
    def parallel_edge_processing(self, edge_for_label, reference):
        """使用多进程处理 edges"""
        with Manager() as manager:
            edges = manager.dict()  # 使用共享内存字典
            for node_pire in edge_for_label:
                edges[node_pire] = manager.list()  # 初始化共享内存列表

            tasks = [(node_pire, edge, reference) for node_pire, edge in edge_for_label.items()]
            
            with Pool(processes=32) as pool:
                results = pool.map(self.process_edge, tasks)

            # 更新 edges
            for node_pire, value in results:
                edges[node_pire].append(value)
            edges = {key: float(value[0]) for key, value in edges.items()}
